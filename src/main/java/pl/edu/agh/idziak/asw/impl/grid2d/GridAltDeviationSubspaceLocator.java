 ...
 */
public class GridAltDeviationSubspaceLocator implements DeviationSubspaceLocator<GridInputPlan, GridCollectiveState> {
        ...
        for (GridEntityState state : bucket.states) {
            for (GridEntityState otherState : otherBucket.states) {
                if (state.equals(otherState))
                    continue;
                if (acc.inputPlan.getDistanceHeuristic().estimateHeuristicDistanceForEntity(state, otherState) > WARNING_PROXIMITY) {
